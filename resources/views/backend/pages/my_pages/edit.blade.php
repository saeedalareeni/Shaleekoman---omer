            ...
            <div class="card-box">
                <form action="{{route('pages.update', $page->id)}}" method="post" enctype="multipart/form-data">
                        ...
                        <div class="col-md-12">
                            <div class="row">
                                <div class="form-group col-md-3">
                                    ...
                                    <input type="text" class="form-control" placeholder="{{trans('back.name_ar')}}" name="name_ar" value="{{$page->name_ar}}" required>
                                </div>

                                <div class="form-group col-md-3">
                                    <label for="name_en">{{trans('back.name_en')}}</label>
                                    <b class="text-danger">*</b>
                                    <input type="text" class="form-control" placeholder="{{trans('back.name_en')}}" name="name_en" value="{{$page->name_en}}" required>
                                </div>

                                <div class="form-group col-md-3">
                                    <label for="slug">{{trans('back.slug')}}</label>
                                    <b class="text-danger">*</b>
                                    <input id="slug" type="text" class="form-control slug" placeholder="{{trans('back.slug')}}" name="slug" value="{{$page->slug}}" required>
                                </div>

                                <div class="form-group col-md-3">
                                    <label for="status">{{trans('back.Status')}}</label>
                                    <select name="status" class="form-control">
                                        <option value="1" {{ old('status', $page->status) == 1 ? 'selected' : null }}>{{trans('back.Active')}}</option>
                                        <option value="0" {{ old('status', $page->status) == 0 ? 'selected' : null }}>{{trans('back.Inactive')}}</option>
                                    </select>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="col-md-12">
                            <div class="row">
                                <div class="form-group col-md-6">
                                    ...
                                    <textarea id="body_ar" class="d-none" name="body_ar">{{$page->body_ar}}</textarea>
                                    <div class="page-editor-shell editor-ar">
                                        <div id="body_ar_editor">{!! $page->body_ar !!}</div>
                                    </div>
                                </div>

                                <div class="form-group col-md-6">
                                    <label for="body_en">{{trans('back.body_en')}}</label>
                                    <textarea id="body_en" class="d-none" name="body_en">{{$page->body_en}}</textarea>
                                    <div class="page-editor-shell editor-en">
                                        <div id="body_en_editor">{!! $page->body_en !!}</div>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="col-md-12">
                            <div class="row">
                                <div class="form-group col-6">
                                    ...
                                    <textarea class="form-control" name="meta_description_ar" rows="3">{{$page->meta_description_ar}}</textarea>
                                </div>

                                <div class="form-group col-6">
                                    <label for="meta_description_en">{{trans('back.meta_description_en')}}</label>
                                    <textarea class="form-control" name="meta_description_en" rows="3">{{$page->meta_description_en}}</textarea>
                                </div>

                                <div class="form-group col-6">
                                    <label for="meta_keywords_ar">{{trans('back.meta_keywords_ar')}}</label>
                                    <textarea class="form-control" name="meta_keywords_ar" rows="3">{{$page->meta_keywords_ar}}</textarea>
                                </div>

                                <div class="form-group col-6">
                                    <label for="meta_keywords_en">{{trans('back.meta_keywords_en')}}</label>
                                    <textarea class="form-control" name="meta_keywords_en" rows="3">{{$page->meta_keywords_en}}</textarea>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="modal-footer">
                        <button type="submit" class="btn btn-purple">{{trans('back.save_and_update')}}</button>
                    </div>
                </form>
            </div>
        </div>
