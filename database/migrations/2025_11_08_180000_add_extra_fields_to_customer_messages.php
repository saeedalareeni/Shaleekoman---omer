...
class AddExtraFieldsToCustomerMessages extends Migration
{
    public function up()
    {
        Schema::table('customer_messages', function (Blueprint $table) {
            ...
            if (!Schema::hasColumn('customer_messages', 'reply')) {
                $table->text('reply')->nullable()->after('is_replied');
            }
    ...
    {
        Schema::table('customer_messages', function (Blueprint $table) {
            $table->dropColumn(['is_read', 'is_replied', 'reply', 'replied_at']);
        });
    }
}
